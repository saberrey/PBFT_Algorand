...
func NewNode(nodeID string) *Node {
	...
	node := &Node{
		...
		NodeTable: map[string]string{
			...
			//"test7": "localhost:5010",
			//"test8": "localhost:5011",
			//"test9": "localhost:5012",
			//"test10": "localhost:5013",
			//"test11": "localhost:5014",
			//"test12": "localhost:5015",
		},
		View: &View{
			ID: viewID,
			Primary: "Apple",
...
func (node *Node) GetReq(reqMsg *consensus.RequestMsg) error {
	...
	LogStage(fmt.Sprintf("leader prepare to send preprepareMsg (ViewID:%d, SequenceID:%d, Digest:%s ,Request)", node.CurrentState.ViewID, prePrepareMsg.SequenceID,prePrepareMsg.Digest), false)
